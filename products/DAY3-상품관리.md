...
5. 안전인증대상 저장 시 (2022-07-20 추가)
    1. 인증아님 데이터가 등록되는 경우 추가 인증 정보가 등록되지 않아야 함
    2. 기존에 인증 정보가 있는 경우 인증대상 아님 데이터가 등록되면 내용 확인 메세지 호출 후 기존 데이터 사용여부를 N처리하고 인증아님 데이터를 생성해야함
...
### 21) 모든 탭
1. 저장 시 금칙어 확인
2. 등록/MD팀장승인반려/승인완료 이외의 상태에서는 수정 불가능
3. 로그인 유저의 MD에 속해있지 않은 상품은 수정 불가능
